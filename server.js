...
app.get('/api/user-details', (req, res) => {
  if (req.session.isAuthenticated && req.session.user) {
    const { username, email, phone, bio, profilePic, bname, date, bdes, btel, logo, bemail, gender, business, blocation} = req.session.user;

    res.json({ username, email, phone, bio, profilePic, date, bname, bdes, logo, btel, bemail, gender, business, blocation });
  } else {
...
// Route to fetch a specific user's details
app.get('/api/user/:id', (req, res) => {
  ...
  if (user) {
    const profileHtml = `
      ...
      <body>
        <div class="profile">
          <div class="img" style="background-image: url('${user.profilePic}');" alt="${user.username}'s profile picture"></div>
          <h2>${user.username}</h2>
          <p>Email: ${user.email}</p>
          <p>Phone: ${user.phone}</p>
          <p>Gender: ${user.gender}</p>
          <p>Type of Business: ${user.business}</p>
          <p>Bio: ${user.bio}</p>
          <p>Business Name: ${user.bname}</p>
          <p>Business Location: ${user.blocation}</p>
          <p>Business Telephone: ${user.btel}</p>
          <p>Business Email: ${user.bemail}</p>
          <p>Date Joined: ${user.date}</p>
          <p>Business Description: ${user.bdes}</p>
